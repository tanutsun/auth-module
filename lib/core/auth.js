...
export default class Auth {
    ...
    getUid(strategy) {
        const _key = this.options.uid.prefix + strategy
    ...
    setUid(strategy, uid) {
        ...
        return this.$storage.setUniversal(_key, uid)
    }
    ...
    getExpire(strategy) {
        const _key = this.options.expire.prefix + strategy

        return this.$storage.getUniversal(_key)
    }

    setExpire(strategy, expire) {
        const _key = this.options.expire.prefix + strategy

        return this.$storage.setUniversal(_key, expire)
    }
    ...
    request(endpoint, defaults) {
        ...
        return this.ctx.app.$axios
            .request(_endpoint)
            .then(response => {
                if (_endpoint.propertyName === "custom") {
                    return response.data
                } else if (_endpoint.propertyName) {
    ...
    requestWith(strategy, endpoint, defaults) {
        ...
        if (_endpoint.propertyName === "custom") {
            _endpoint.url = _endpoint.url + '/' + this.getUid(strategy);
        }
